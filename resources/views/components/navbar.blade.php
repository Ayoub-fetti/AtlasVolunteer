<nav class="bg-white shadow-md">
    <div class="container mx-auto px-4">
                ...
                @guest
                    ...
                    <div class="relative group">
                        ...
                        <div class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 z-10">
                            <a href="{{ route('register.form', ['role' => 'volunteer']) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50">Register as Volunteer</a>
                            <a href="{{ route('register.form', ['role' => 'organization']) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-50">Register as Organization</a>
                        </div>
                    </div>
                @else
                    <form action="{{ route('logout') }}" method="POST" class="m-0">
                        @csrf
                        <button type="submit" class="text-gray-600 hover:text-indigo-600 transition">Logout</button>
                    </form>
                @endguest
            </div>
        </div>
        ...
        <div id="mobileMenu" class="md:hidden hidden flex-col space-y-4 py-4">
            ...
            <a href="{{ route('home') }}" class="block text-gray-700 hover:text-indigo-600">Opportunities</a>
            <a href="{{ route('donation.index') }}" class="block text-gray-700 hover:text-indigo-600">Donations</a>
            <a href="/contact" class="block text-gray-700 hover:text-indigo-600">Contact</a>
            @auth
                <a href="{{ auth()->user()->role === 'volunteer' ? route('profile.index') : route('organization.index') }}" class="block text-gray-700 hover:text-indigo-600">Profile</a>
                <a href="{{ route('messages.index') }}" class="block text-gray-700 hover:text-indigo-600">Messages</a>
            @endauth
            @guest
                <a href="{{ route('login.form') }}" class="block text-gray-700 hover:text-indigo-600">Login</a>
                <a href="{{ route('register.form', ['role' => 'volunteer']) }}" class="block text-gray-700 hover:text-indigo-600">Register as Volunteer</a>
                <a href="{{ route('register.form', ['role' => 'organization']) }}" class="block text-gray-700 hover:text-indigo-600">Register as Organization</a>
            @else
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="block text-left text-gray-700 hover:text-indigo-600 w-full">Logout</button>
                </form>
            @endguest
        </div>
    </div>
