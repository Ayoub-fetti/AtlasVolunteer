            ...
            <div class="p-6">
                ...
                @if($conversations->isEmpty())
                    <div class="text-center py-8">
                        ...
                        <p class="mt-2 text-sm font-medium text-gray-500">You have no conversations yet</p>
                    </div>
                @else
                                    ...
                                    @if($conversation->user_id == auth()->id())
                                        @if($conversation->receiver->role == 'organization')
                                            <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200" 
                                                src="{{ optional($conversation->receiver->organizations->first())->logo ? asset('storage/' . $conversation->receiver->organizations->first()->logo) : asset('images/default-avatar.png') }}" 
                                                alt="{{ $conversation->receiver->name }}">
                                        @else
                                            <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200" 
                                                src="{{ optional($conversation->receiver->volunteer)->profile_picture ? asset('storage/' . $conversation->receiver->volunteer->profile_picture) : asset('images/default-avatar.png') }}" 
                                                alt="{{ $conversation->receiver->name }}">
                                        @endif
                                    @else
                                        @if($conversation->user->role == 'organization')
                                            <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200" 
                                                src="{{ optional($conversation->user->organizations->first())->logo ? asset('storage/' . $conversation->user->organizations->first()->logo) : asset('images/default-avatar.png') }}" 
                                                alt="{{ $conversation->user->name }}">
                                        @else
                                            <img class="h-12 w-12 rounded-full object-cover border-2 border-gray-200" 
                                                src="{{ optional($conversation->user->volunteer)->profile_picture ? asset('storage/' . $conversation->user->volunteer->profile_picture) : asset('images/default-avatar.png') }}" 
                                                alt="{{ $conversation->user->name }}">
                                        @endif
                                    @endif
                                </div>
                ...
                <div class="mt-8 pt-6 border-t border-gray-200">
                    <h2 class="text-lg font-medium text-gray-900 mb-4">Start a new conversation</h2>
                    <form action="{{ route('conversations.create') }}" method="POST">
                        @csrf
                        <div class="flex flex-col sm:flex-row gap-4">
                            <div class="flex-grow">
                                <label for="receiver_id" class="sr-only">Select contact</label>
                                <select name="receiver_id" id="receiver_id" required
                                    ...
                                    <option value="">Select a person to chat with</option>
